 ...
 */
// r - записівает в конец
// w - записівакт с обнулением
...
function myDiff()
{
    $resource1 = fopen('x.txt', 'r+');
    $xData = fgets($resource1);
    $xData = explode(' ', trim($xData));
    
    $resource2 = fopen('y.txt', 'r+');
    $yData = fgets($resource2);
    $yData = explode(' ', trim($yData));
    
    $onlyFirst = array_diff($xData, $yData);
    $onlyFirst = array_unique($onlyFirst);
    $r1 = fopen('only_first.txt', 'a+');
    fwrite($r1, implode(' ', $onlyFirst));
    
    
    $both = array_intersect($xData, $yData);
    $both = array_unique($both);
    fwrite(fopen('both,txt', 'a+'), implode(' ', $both));
...
function myWordsOrder()
{
    ...
    fwrite(fopen('words.txt', 'w'), implode(' ', $words));
