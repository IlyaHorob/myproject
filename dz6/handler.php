...
if (!empty($_POST)) {
    ...
    if (empty($date1)) {
         ...
         */
        $tmpdate = strtotime('-100 year', $date1Ts);
        if (!empty($format)) {
            echo " 8. ";
            echo date('l', $tmpdate);
            echo '<br>';
        ...
        /**
         * 9.    Создайте html-форму. Добавьте на форму 4 элемента с типом checkbox,
         * сгруппированных с помощью элемента fieldset.
...
/**
 *создать массив форіч в форме.в посте создать єйдж .проверку arraykey exist.переменная массив
 */
