 ...
 * Сделайте так, чтобы первый элемент стал ключом второго элемента, третий элемент - ключом четвертого и так далее.
...
$result = array_revert(array(1, 2, 3, 4, 5, 6));
echo "<pre>";
var_dump($result);
echo "</pre>";
 ...
 * третий на четвертый и наоборот, пятый на шестой и наоборот и так далее. То есть из строки '12345678' нужно сделать '21436587'.
 *
 * @param $str
 *
 * @return string
 */
function str_revet($str)
{
    $length = strlen($str) / 2;
    $result = '';
    $index = 0;
 ...
 * а функция возвращает фамилию и инициалы.
 * Например, передаем Ivanov Ivan Ivanovich и нам выводит: Ivanov I. I.
 *
 * @param string $str
 *
 * @return string
 ...
 * @return string
 */
function multi_table($rows, $colls)
{
    ...
    for ($i = 1; $i <= $rows; $i++) {
        ...
        for ($j = 1; $j <= $colls; $j++) {
            if ($i == 1 || $j == 1) {
                $style = 'style="background-color:green;color:white;height:35px;width:35px"';
...
echo multi_table(5, 5);
echo '<br>';

/**
 * 9.    (+1) Написать рекурсивную функцию, которая на вход получает число и
 * вычисляет и возвращает значение факториала этого числа.
 *
 * @param integer $n
 *
 * @return int
 ...
 * которая принимает на вход параметр (например, $n) и вычисляет число Фибоначчи до предела $n. Результат вычисления возвращается этой же функцией.
 *
 * @param integer $num
 *
 * @return int
